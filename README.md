...
npm run policy:template-mfes-import-boundary  # ADR-0003 consumer half - no template may import a concrete @gears-frontx/mfes name
npm run policy:guideline-index
```

`policy:template-pin-drift` runs in one of two modes, for TEMPLATE pin sites:
...
This repo's own root `package.json` is a separate case, checked the same way regardless of mode: its `@gears-frontx/cli` devDependency installs the *published* CLI (so `validate:templates` can run it), not a template dependency, so it is always verified directly against the npm registry - never against a sibling checkout, which may well have an unpublished `develop` version of the CLI that this repo's `npm ci` could never install.
