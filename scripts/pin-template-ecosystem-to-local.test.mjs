...
describe('fail-closed manifest reads (review finding)', () => {
  it('planPinLocalization fails closed, naming the file, when a manifest under the template is malformed JSON', async () => {
    const root = await makeRoot();
    await writePinnedPackage(root);
    await mkdir(path.join(root, 'template-shell'), { recursive: true });
    await writeFile(path.join(root, 'template-shell', 'package.json'), '{ "name": broken');

    expect(() => planPinLocalization({ repoRoot: root })).toThrow(/cannot parse .*template-shell.*package\.json/);
  });

  it('applyPinLocalization writes from the manifest object the plan already parsed, never re-reading the file from disk', async () => {
    const root = await makeRoot();
    await writePinnedPackage(root);
    await writeJson(path.join(root, 'template-shell', 'package.json'), {
      name: '@gears-frontx/frontx-template-shell',
    ...
    // Corrupt the file on disk AFTER planning. A second read (the bug this
    // guards against) would throw trying to parse this; reusing the
    // already-parsed object instead means `apply` never looks at it again.
    await writeFile(path.join(root, 'template-shell', 'package.json'), 'not valid json at all {{{');

    expect(() => applyPinLocalization(plan.manifestEdits, root)).not.toThrow();

    const manifest = asManifest(await readJson(path.join(root, 'template-shell', 'package.json')));
    expect(manifest.dependencies['@gears-frontx/mfes']).toBe('file:../packages/mfes');
  });

  it('applyPinLocalization fails closed, naming the file and field, rather than throwing a bare TypeError, when a manifest edit names a field that is not an object', async () => {
    const root = await makeRoot();
    const manifestPath = path.join(root, 'template-shell', 'package.json');
    await writeJson(manifestPath, {
      name: '@gears-frontx/frontx-template-shell',
      // "dependencies" deliberately a string, not an object - simulates a
      // ManifestEdit built (or a manifest that changed shape) independently
      // of a real `planPinLocalization` run, which this repo's own fixtures
      // can never produce through the normal flow.
    ...
    expect(() => applyPinLocalization(manifestEdits, root)).toThrow(
      /has no "dependencies" object to set @gears-frontx\/mfes on/,
    );
  });

  it('overrides conflict detection reads from the already-parsed manifest, not a second disk read', async () => {
    const root = await makeRoot();
    await writePinnedPackage(root);
    await writeJson(path.join(root, 'template-shell', 'package.json'), {
      name: '@gears-frontx/frontx-template-shell',
    ...
    // Same value as the plan would compute - no conflict, and this must
    // succeed by reading the "overrides" this test just wrote, not by
