 ...
 * *published* alpha, so edits to the sibling checkout's `packages/*` never
 * reach the template and the failure is silent - the template builds,
 * type-checks and tests green against code nobody changed.
 *
...
export function linkEcosystemPackages({
  ...
  for (const name of packageDirs) {
    ...
    // Deliberately NOT `readJsonFileOrThrow` (template-ecosystem-packages.mjs):
    // that helper always reads through real `node:fs`, while this function's
    // whole rollback-safety test suite depends on an INJECTED `fs` (the
    // `fs` parameter above) to simulate a read failure without touching a
    // real filesystem. Failing closed here means returning `{ ok: false }`
    // through the same result channel every other failure in this function
    // uses, not throwing - a second reason a throwing helper would not fit
    // this call site without its own wrapper.
    /** @type {unknown} */
    let manifest;
    try {
