...
describe('runCli', () => {
  ...
  describe('registry mode (no FRONTX_ECOSYSTEM_DIR)', () => {
    ...
    // silently skipping this repo's own root package.json pins entirely.
    // Covered now via the unconditional root-manifest check `check()` runs
    // in BOTH modes - see "root manifest pins are always a registry
    // question" below for the sibling-mode half of that fix.
  ...
  // Fork-dry-run finding: the root manifest's own pins must be verified
  // against the REGISTRY, unconditionally - never sibling-checkout truth,
  // in EITHER mode. The root's `@gears-frontx/cli` devDependency installs
  // the published CLI (so validate-templates.mjs can run it), not a
  // template dependency, and it cannot follow whatever version a sibling
  // `gears-frontx` checkout has on disk (that checkout's `packages/cli` is
  // routinely an unpublished `develop` snapshot this repo's `npm ci` could
  // never install). Comparing it against sibling truth produced a false
  // "drifted" finding the moment the sibling's in-development CLI moved
  // past whatever is actually published - exactly what a real fork dry run
  // against gears-frontx develop hit.
  describe("root manifest pins are always a registry question, in both modes", () => {
    it('sibling mode still verifies the root manifest against the registry, not the sibling checkout', async () => {
      const root = await makeRoot();
      const ecosystemDir = path.join(root, 'ecosystem');
      // The sibling checkout's own @gears-frontx/cli is AHEAD of what the
      // root manifest pins - if the root pin were compared against this
      // truth (the old, wrong behavior) this would report a false drift.
      await writeEcosystemPackages(ecosystemDir, { api: '0.3.0-alpha.0' });
      await writeJson(path.join(ecosystemDir, 'packages', 'cli', 'package.json'), {
        name: '@gears-frontx/cli',
        version: '0.3.0-alpha.5',
      });
      await writeJson(path.join(root, 'package.json'), {
        name: 'gears-frontx-templates',
        devDependencies: { '@gears-frontx/cli': '0.3.0-alpha.4' },
      });
      await writeManifest(path.join(root, 'template-shell'));
      await writeJson(path.join(root, 'template-shell', 'package.json'), {
        dependencies: { '@gears-frontx/api': '0.3.0-alpha.0' },
      ...
      const seen = [];
      const { exitCode, output } = run(root, {
        env: { FRONTX_ECOSYSTEM_DIR: ecosystemDir },
        npmViewVersion: (name, version) => {
          seen.push(`${name}@${version}`);
          return name === '@gears-frontx/cli' && version === '0.3.0-alpha.4';
        },
      });

      expect(exitCode).toBe(0);
      expect(seen).toEqual(['@gears-frontx/cli@0.3.0-alpha.4']);
      expect(output).not.toContain('actual 0.3.0-alpha.5');
    });

    it('sibling mode fails the root manifest pin when the registry says it does not exist - never by comparing to sibling truth', async () => {
      const root = await makeRoot();
      const ecosystemDir = path.join(root, 'ecosystem');
      await writeEcosystemPackages(ecosystemDir, { api: '0.3.0-alpha.0' });
      await writeJson(path.join(root, 'package.json'), {
      ...
      const { exitCode, output } = run(root, {
        env: { FRONTX_ECOSYSTEM_DIR: ecosystemDir },
        npmViewVersion: () => false,
      });

      expect(exitCode).toBe(1);
      expect(output).toContain('@gears-frontx/cli');
      expect(output).toContain('pinned 0.3.0-alpha.99, not found on the npm registry');
      // Never the sibling-truth "drifted" framing for this site.
      expect(output).not.toContain('@gears-frontx/cli"]: pinned 0.3.0-alpha.99, actual');
    });

    it('registry mode verifies the root manifest against the registry too (unchanged by this fix)', async () => {
      const root = await makeRoot();
      await writeJson(path.join(root, 'package.json'), {
        name: 'gears-frontx-templates',
        devDependencies: { '@gears-frontx/cli': '0.3.0-alpha.4' },
      ...
      const { exitCode } = run(root, {
        env: {},
        npmViewVersion: (name, version) => name === '@gears-frontx/cli' && version === '0.3.0-alpha.4',
      });
