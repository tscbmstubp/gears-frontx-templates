...
export function planPinLocalization({ repoRoot, ecosystemRoot = repoRoot }) {
  ...
  for (const [relFile, subs] of byFile) {
    ...
    // second time there. A second read invites the plan and the write to see
    // two different files (however unlikely between two lines of one
    // synchronous script) and, worse, throws a bare uncaught `TypeError` if
    // `manifest[field]` turns out not to be the object this plan already
    // confirmed it was - re-reading is exactly what could make that false.
    ...
    if (relFile === 'package.json') {
      const existingOverrides = readExistingOverrides(manifest);
      overrides = [];
...
/**
 * @param {Record<string, unknown>} manifest already-parsed, from
 *   `readPackageManifest` - never re-read from disk here (see
 *   `planPinLocalization`'s own comment on why re-reading is the risk this
 *   avoids).
 * @returns {Record<string, string>}
 */
function readExistingOverrides(manifest) {
  const overrides = manifest['overrides'];
  if (typeof overrides !== 'object' || overrides === null || Array.isArray(overrides)) return {};
  return /** @type {Record<string, string>} */ (overrides);
}
 ...
 *
 * Mutates and re-serializes the SAME `manifest` object `planPinLocalization`
 * already read and validated - it does not re-read the file. Re-reading here
 * used to be both pointless (the object was already in hand) and a real
 * risk: a second read that disagreed with the first, however unlikely within
 * one synchronous script, would let `manifest[field][packageName] = fileSpec`
 * throw a bare, uncaught `TypeError` the moment `field` was not the object
 * shape the plan (built from the FIRST read) had already confirmed.
 *
...
export function applyPinLocalization(manifestEdits, repoRoot) {
  ...
  for (const { manifestPath, manifest, edits, overrides } of manifestEdits) {
    ...
    for (const { field, packageName, pinnedVersion, fileSpec } of edits) {
      // Fail-closed rather than a bare TypeError: `field` came from a real
      // pin site `scanTreePins` found IN this exact manifest object, so it
      // is always already the right shape in practice - this guard is the
      // difference between a clear message and a crash on the day that
      // stops being true (a plan and manifest that somehow disagree, a
      // future caller constructing a `ManifestEdit` by hand).
      const fieldValue = manifest[field];
      if (typeof fieldValue !== 'object' || fieldValue === null || Array.isArray(fieldValue)) {
        throw new Error(
          `Cannot localize: ${reportedFile} has no "${field}" object to set ${packageName} on - ` +
