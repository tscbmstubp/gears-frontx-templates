 ...
 *
 * This mode split governs TEMPLATE pin sites only. This repo's own root
 * `package.json` (its `@gears-frontx/cli` devDependency) is verified against
 * the registry unconditionally regardless of which mode this function
 * returns - see `rootNpmViewVersion` in `template-pin-drift-check.mjs`'s
 * `check()`. That pin installs the published CLI this repo's own tooling
 * runs, not a template's dependency, and it cannot follow whatever version a
 * sibling checkout happens to have on disk.
 *
...
/**
 * Reads and parses one JSON file. FAILS CLOSED on every way the operation can
 * fail - an unreadable file, unparseable JSON, a non-object body - each
 * aborting with a message NAMING the file rather than escaping as node's raw
 * `ENOENT`/`SyntaxError`, which names neither the file's role nor the tool
 * that wanted it (so a red build reads as a broken script instead of a
 * broken manifest).
 *
 * The one JSON-reading primitive every repo-script that reads a
 * `package.json` (or `frontx-template.json`, or an `overrides` block) builds
 * on, so "unreadable" and "malformed" fail the same way everywhere instead
 * of each caller inventing its own (a bare `JSON.parse(fs.readFileSync(...))`
 * used to reappear per call site, each one throwing node's raw error with no
 * indication of which file or why it was being read).
 *
 * `purpose` is a trailing clause naming what the caller needed the file for
 * (e.g. `"no pin site it declares or defines can be checked."`), appended
 * after the failure reason so the message stays specific to the caller's own
 * situation without duplicating the fail-closed mechanics.
 *
 * @param {string} filePath
 * @param {string} purpose
 * @returns {Record<string, unknown>}
 */
export function readJsonFileOrThrow(filePath, purpose) {
  /** @type {string} */
  let raw;
  try {
    raw = fs.readFileSync(filePath, 'utf8');
  } catch (error) {
    throw new Error(`cannot read ${filePath} (${describeError(error)}) - ${purpose}`);
  }
  ...
  try {
    ...
    throw new Error(`cannot parse ${filePath} as JSON (${describeError(error)}) - ${purpose}`);
  }

  if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
    throw new Error(`${filePath} is not a JSON object - ${purpose}`);
  }
  return /** @type {Record<string, unknown>} */ (parsed);
}

/**
 * `readJsonFileOrThrow` specialized to a `package.json` (or any manifest
 * whose pin sites this repo's guards inspect) - see that function for the
 * fail-closed rules this inherits.
 *
 * @param {string} packageJsonPath
 * @returns {Record<string, unknown>}
 */
export function readPackageManifest(packageJsonPath) {
  return readJsonFileOrThrow(packageJsonPath, 'no pin site it declares or defines can be checked.');
}
 ...
 *
 * @param {string} rootDir the ecosystem checkout root (where packages/* lives now)
